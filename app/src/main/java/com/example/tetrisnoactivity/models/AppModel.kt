...
class AppModel {
    ...
    var currentState: String = Statuses.AWAITING_START.name // TODO: Why String ?
    ...
    fun generateField(action: String){
        if (isGameActive()) {
            ...
            when (action) {
                Motions.LEFT.name -> coordinate?.x = coordinate?.x?.minus(1)
                Motions.RIGHT.name -> coordinate?.x = coordinate?.x?.plus(1)
                Motions.DOWN.name -> coordinate?.y = coordinate?.y?.plus(1)
                Motions.ROTATE.name -> {
    ...
    private fun resetField(ephermeralCellsOnly: Boolean = true){
        for (i in 0 until FieldConstants.ROW_COUNT.value) {
            (0 until FieldConstants.COLUMN_COUNT.value).filter {
                !ephermeralCellsOnly || field[i][it] == CellConstants.EPHEMERAL.value
    ...
    private fun shiftRows(nToRow: Int){
        /// Shift all blocks above completed line on one row down
        if (nToRow > 0)
            for (i in (nToRow - 1) downTo 0)
                for(j in 0 until field[i].size)
                    setCellStatus(i+1, j, getCellStatus(i, j))

        // TODO: Why do we clear the first row?
        ...
        return if (position.y < 0 || position.x < 0)
            ...
            for (i in 0 until shape.size)
                for (j in 0 until shape[i].size){
                    ...
                    val x = position.x + j
                    if (CellConstants.EMPTY.value != shape[i][j] &&
                        CellConstants.EMPTY.value != getCellStatus(y, x))
                            return false
                }
            true
        }
    ...
    fun setCellStatus(row: Int, column: Int, newStatus: Byte?){
        if (newStatus != null)
            field[row][column] = newStatus
    }
}